...
function toFigma(list) {
    ...
    utilities_1.Fs.writeSync(jsonFile, JSON.stringify(colors));
...
async function css(to, colors, useRoots) {
    ...
    const run = async (value, key, item) => {
        if (!(typeof value == "string" || typeof value == "number"))
            throw SyntaxError("Error in code - cannot have a color shade on a shade at key - " +
                key +
                " " +
                typeof value);
    ...
    for (const item of Object.keys(colors)) {
        if (typeof colors[item] === "object")
            ...
            await run(colors[item], "default", item);
    }
    utilities_1.Fs.createPath(to, { content: list.join("\n") });
}
const PREFIX = "--color-";
